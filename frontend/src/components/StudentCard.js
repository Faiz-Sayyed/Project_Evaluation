  ...
  return (
    <div className="mb-5 p-5 bg-gray-200 drop-shadow-lg rounded-md">
      <div className="flex justify-between w-full">
        ...
        {mentor.hasSubmitted === false && (
          <div className="flex justify-end w-1/3">
            {isSelected ? (
              <div className="lg:flex lg:justify-between">
                <div
                  onClick={removeStudent}
                  className="mx-1 px-3 py-1 text-white bg-red-500 rounded-lg border hover:cursor-pointer hover:bg-red-600"
                >
                  Remove
                </div>
                ...
                <div>
                  {editing === student._id ? (
                    <div
                      onClick={saveScores}
                      className="mt-1 lg:mt-0 mx-1 px-3 py-1 bg-green-500 text-white rounded-lg border hover:cursor-pointer hover:bg-green-600"
                    >
                      Save
                    </div>
                  ) : (
                    <div
                      onClick={() => setEditing(student._id)}
                      className="mt-1 lg:mt-0 mx-1 px-3 py-1 bg-gray-300 rounded-lg border hover:cursor-pointer hover:bg-gray-400"
                    >
                      Edit
                    </div>
                  )}
              ...
              <div
                onClick={addStudent}
                className="px-3 py-1 bg-green-500 text-white rounded-lg border hover:cursor-pointer hover:bg-green-600"
              >
                Add
              </div>
            )}
      ...
      {mentor._id !== student.mentorID && (
        <div className="flex w-2/3 text-xl md:text-lg">
          ...
          <div className="ml-3 font-semibold">
            {student.mentorName ? student.mentorName : "N/A"}
          </div>
        </div>
      )}
      ...
      <div className="flex text-3xl md:text-2xl mt-3">
        <div>Total Marks:</div>
        <div className="ml-3 font-semibold">{student.totalMarks}</div>
      </div>
    </div>
  );
};
